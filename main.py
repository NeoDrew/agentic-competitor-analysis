...
async def main() -> int:
    ...
    print(f"Sentinel is locking onto: {args.url}")
    print(f"Searching archives for data from {args.months} months ago...\n")

    # 1. Fetch Current State (Async)
    try:
        new_md = await get_current_state(args.url)
    ...
    old_md, snap = get_historical_state(args.url, args.months)
    ...
    if old_md is None:
        print(f"No Wayback snapshot found for ~{args.months} months ago.")
    ...
    try:
        ...
        gemini_resp = analyze_diff(old_md, new_md)
